...
RECIPIENT_EMAIL  = "ajay.kumar@incedoinc.com"   # ← replace with your email address
MONITORING_KEYWORDS = [
...
def build_html(data, report_date, emails_found):
    ...
    # ── critical alert callout — grouped by type → env → db ──────────────────

    # collect critical items per type
    crit_ts = defaultdict(lambda: defaultdict(list))   # env -> db -> [ts]
    crit_dg = defaultdict(lambda: defaultdict(list))   # env -> db -> [dg]
    crit_mp = defaultdict(lambda: defaultdict(list))   # env -> host -> [mp]

    for ts in _dedupe(data.tablespaces, key=lambda e: (e.database, e.tablespace_name)):
        if ts.used_percent >= TS_CRIT:
            crit_ts[_env(ts.database)][ts.database].append(ts)
    for dg in _dedupe(data.diskgroups, key=lambda e: (e.database, e.diskgroup_name)):
        if dg.used_percent >= DG_CRIT:
            crit_dg[_env(dg.database)][dg.database].append(dg)
    for mp in _dedupe(data.mount_points, key=lambda e: (e.host, e.mount_point)):
        if mp.used_percent >= MP_CRIT:
            host_key = mp.host if (mp.host and mp.host.strip()) else "Unknown Host"
            crit_mp[_env(host_key)][host_key].append(mp)

    has_any = bool(crit_ts or crit_dg or crit_mp)

    def _alert_type_banner(label):
        return (
            '<tr><td colspan="4" style="padding:7px 10px 5px;background:#922b21;' +
            'color:#fff;font-size:12px;font-weight:700;border:1px solid #7b241c;' +
            f'letter-spacing:0.5px;">{label}</td></tr>'
        )

    def _alert_env_banner(env):
        ecol = ENV_COLORS.get(env, "#34495e")
        return (
            f'<tr><td colspan="4" style="padding:6px 10px 4px;background:{ecol};' +
            f'color:#fff;font-size:11px;font-weight:700;border:1px solid {ecol};' +
            f'letter-spacing:0.4px;">&#128193; {env} Environment</td></tr>'
        )

    def _alert_db_banner(name):
        return (
            '<tr><td colspan="4" style="padding:5px 10px 4px;background:#fdecea;' +
            'font-size:11px;font-weight:700;color:#922b21;border:1px solid #f1948a;' +
            f'letter-spacing:0.3px;">&#128194; {name}</td></tr>'
        )
    ...
    if has_any:
        ...
        # Tablespaces
        if crit_ts:
            alert_rows.append(_alert_type_banner("&#128451; Tablespaces"))
            for env in sorted_envs:
                if env not in crit_ts:
                    continue
                alert_rows.append(_alert_env_banner(env))
                for db_name in sorted(crit_ts[env].keys()):
                    alert_rows.append(_alert_db_banner(db_name))
                    for ts in sorted(crit_ts[env][db_name], key=lambda x: x.used_percent, reverse=True):
                        alert_rows.append(_td(
                            (f'<b style="color:#c0392b">{ts.tablespace_name}</b>',
                             "font-family:monospace;font-size:11px;"),
                            (ts.database, "color:#555;"),
                            (f"{ts.used_percent:.1f}%",
                             "text-align:center;font-weight:700;color:#c0392b;"),
                            (_badge(*_status(ts.used_percent, TS_CRIT, TS_WARN)),
                             "text-align:center;"),
                        ))

        # Diskgroups
        if crit_dg:
            alert_rows.append(_alert_type_banner("&#128189; Diskgroups (ASM)"))
            for env in sorted_envs:
                if env not in crit_dg:
                    continue
                alert_rows.append(_alert_env_banner(env))
                for db_name in sorted(crit_dg[env].keys()):
                    alert_rows.append(_alert_db_banner(db_name))
                    for dg in sorted(crit_dg[env][db_name], key=lambda x: x.used_percent, reverse=True):
                        alert_rows.append(_td(
                            (f'<b style="color:#c0392b">{dg.diskgroup_name}</b>', ""),
                            (dg.database, "color:#555;"),
                            (f"{dg.used_percent:.1f}%",
                             "text-align:center;font-weight:700;color:#c0392b;"),
                            (_badge(*_status(dg.used_percent, DG_CRIT, DG_WARN)),
                             "text-align:center;"),
                        ))

        # Mount Points
        if crit_mp:
            alert_rows.append(_alert_type_banner("&#128190; Mount Points"))
            for env in sorted_envs:
                if env not in crit_mp:
                    continue
                alert_rows.append(_alert_env_banner(env))
                for host_name in sorted(crit_mp[env].keys()):
                    alert_rows.append(_alert_db_banner(host_name))
                    for mp in sorted(crit_mp[env][host_name], key=lambda x: x.used_percent, reverse=True):
                        alert_rows.append(_td(
                            (f'<code style="color:#c0392b;">{mp.mount_point}</code>', ""),
                            (host_name, "color:#555;"),
                            (f"{mp.used_percent:.0f}%",
                             "text-align:center;font-weight:700;color:#c0392b;"),
                            (_badge(*_status(mp.used_percent, MP_CRIT, MP_WARN)),
                             "text-align:center;"),
                        ))
...
# ── email with attachment (via Outlook COM) ───────────────────────────────────
def _send_with_attachment(to_addr, report_date, html_body, attachment_path):
    """
    Send the dashboard email via Outlook COM with:
      - HTML body  (renders in most email clients)
      - HTML file  attached  (exact copy, opens in browser)
    """
    try:
    ...
    mail.To      = to_addr
    mail.Subject = f"📊 {day_name} Monitoring Dashboard ({report_date})"
    mail.HTMLBody = html_body                 # full colour-coded HTML body

    # attach the saved HTML file so recipient can open it locally in browser
...
def run(folder_name, days_back, output_file, send_to, open_browser):
    ...
    # 8. Email — always send to RECIPIENT_EMAIL; also to --send-to if given
    recipients = list({r for r in [send_to, RECIPIENT_EMAIL] if r and "@" in r})
    for addr in recipients:
        print(f"\n  📧 Sending to {addr} (with HTML attachment)...")
        try:
            _send_with_attachment(addr, report_date, html, output_file)
            print("  ✅ Email sent")
        except Exception as e:
